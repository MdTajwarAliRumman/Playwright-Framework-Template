...
test.describe('Register Checkout Flow', () => {
    ...
    test('Verify Register while checkout', async ({ page }) => {
        ...
        await test.step(" Verify Invoice was downloaded successfully!", async () => {
            await expect(allProductsPage.downloadInvoice).toBeVisible();
            await allProductsPage.downloadInvoice.click();
        })
